...
namespace MarkdownDeepBenchmark {
	...
	class Program {
		...
		/// <summary>
		///     executes a standard benchmark on short, medium, and long markdown samples use this to verify any impacts of
		///     code changes on performance please DO NOT MODIFY the input samples or the benchmark itself as this will invalidate
		...
		/// <summary>
		///     performs a rough benchmark of the Markdown engine using small, medium, and large input samples please DO NOT
		///     MODIFY the input samples or the benchmark itself as this will invalidate previous benchmark runs!
		/// </summary>
		static void Benchmark(string text, int iterations) {
			...
			if (iterations == 1) {
				...
				Console.WriteLine(" (" + Convert.ToDouble(sw.ElapsedMilliseconds)/Convert.ToDouble(iterations) +
								  " ms per iteration)");
			}
		}
	}
