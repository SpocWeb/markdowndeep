...
namespace MarkdownDeep {
	...
	internal static class Utils {
		...
		static readonly char[] _LINE_ENDS = {'\r', '\n'};
		...
		public static void Push<T>(this List<T> list, T value) where T : class => list.Add(value);
		...
		public static bool IsInList(string str, string[] list) {
			foreach (string t in list) {
				if (string.CompareOrdinal(t, str) == 0) {
					return true;
				}
			}
			return false;
		...
		/// <summary> Check if a character is escapable in markdown </summary>
		public static bool IsEscapableChar(char ch, bool isExtraMode) {
			switch (ch) {
				case '\\':
				case '`':
				case '*':
		...
		public static void SkipEscapableChar(this StringScanner p, bool isExtraMode) {
			if (p.Current == '\\' && IsEscapableChar(p.CharAtOffset(1), isExtraMode)) {
				p.SkipForward(2);
			} else {
				p.SkipForward(1);
			}
		...
		public static string UnescapeString(string str, bool isExtraMode) {
			...
			for (int i = 0; i < str.Length; i++) {
				if (str[i] == '\\' && i + 1 < str.Length && IsEscapableChar(str[i + 1], isExtraMode)) {
					b.Append(str[i + 1]);
					i++;
				} else {
					b.Append(str[i]);
		...
		public static string NormalizeLineEnds(string str) {
			if (str.IndexOfAny(_LINE_ENDS) < 0) {
				return str;
			}
		...
		public static bool IsWebAddress(string str) => str.StartsWith("http://") ||
		...
		public static bool IsUrlFullyQualified(string url) => url.Contains("://") || url.StartsWith("mailto:");
	}
}